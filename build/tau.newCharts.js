(function(definition) {
    ...
    var node$visitor$factory$$TNodeVisitorFactory = (function () {
        ...
        var TNodeMap = {
            ...
            'WRAP.MULTI_AXES': function(node, continueTraverse) {
                ...
                var xItems = _(node.x).filter(function(m)  {return m.length > 0}).reverse();
                var yItems = _(node.y).filter(function(m)  {return m.length > 0}).reverse();
    ...
    //            var Y_AXIS_POS = [0 - y.padding, 0];
                ...
                _(xItems).each(function(items, lvl)  {
                    var level = lvl;
                ...
                _(yItems).each(function(items, lvl)  {
                    var level = lvl;
                    var myH = H / items.length;
                    _(items).each(function(y, i)  {
    ...
    dsl$reader$$DSLReader.prototype = {
        ...
        traverse: function (rawData, styleEngine) {
            ...
            var isLCol = function(r, c)  {return c === 0} ;
            var isTRow = function(r, c)  {return r === 0} ;
    
            var transformationExtractAxes = (function(root)  {
    
                var traverse = (function(node, refWrapper) 
                {
                    if (!node.$matrix) {
                        return node;
                    }
