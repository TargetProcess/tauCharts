...
define('unit-domain-mixin',["exports", "./unit-domain-period-generator", "./utils/utils", "underscore", "d3"], function (exports, _unitDomainPeriodGenerator, _utilsUtils, _underscore, _d3) {
  ...
  var rangeMethods = {
    ...
    linear: function (inputValues, interval, props) {
      ...
      var range = [Math.min(min, domainParam[0]), Math.max(max, domainParam[1])];
...
define('utils/utils-draw',["exports", "../utils/utils", "../formatter-registry"], function (exports, _utilsUtils, _formatterRegistry) {
  ...
  var decorateTickLabel = function (nodeScale, x) {
    nodeScale.selectAll(".tick text").attr("transform", rotate(x.guide.rotate)).style("text-anchor", x.guide.textAnchor);
  };

  var fnDrawDimAxis = function (x, AXIS_POSITION, size) {
    var container = this;
    if (x.scaleDim) {
      var axisScale = d3.svg.axis().scale(x.scaleObj).orient(x.guide.scaleOrient).ticks(_.max([Math.round(size / x.guide.density), 4]));
  ...
  var fnDrawGrid = function (node, H, W) {
    ...
    if (linesOptions.length > 0) {
      ...
      if ((linesOptions.indexOf("x") > -1) && node.x.scaleDim) {
        var x = node.x;
        var xGridAxis = d3.svg.axis().scale(x.scaleObj).orient(x.guide.scaleOrient).tickSize(H).ticks(_.max([Math.round(W / x.guide.density), 4]));
      ...
      if ((linesOptions.indexOf("y") > -1) && node.y.scaleDim) {
        var y = node.y;
        var yGridAxis = d3.svg.axis().scale(y.scaleObj).orient(y.guide.scaleOrient).tickSize(-W).ticks(_.max([Math.round(H / y.guide.density), 4]));
    ...
    _classProps(Plot, null, {
      ...
      _autoDetectDimensions: {
        ...
        value: function (data) {
          ...
          return _.reduce(data, function (dimMemo, rowItem) {
...
define('charts/tau.chart',["exports", "./tau.plot", "../utils/utils"], function (exports, _tauPlot, _utilsUtils) {
