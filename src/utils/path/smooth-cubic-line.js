export default function getSmoothCubicLine(points) {
    ...
    if (points.length === 2) {
        return [
            points[0],
            interpolatePoint(points[0], points[1], 1 / 3),
            interpolatePoint(points[0], points[1], 2 / 3),
            points[1]
        ];
    }
    ...
    for (var i = 2; i < points.length; i++) {
        ...
        if ((p1.x - p0.x) * (p2.x - p1.x) === 0) {
            ...
            c1y = p1.y - interpolateValue(p1.y - p0.y, (p2.y - p1.y) / (p2.x - p1.x) * (p1.x - p0.x), 1 / 2) / 3;
            c2y = p1.y + interpolateValue(p2.y - p1.y, (p1.y - p0.y) / (p1.x - p0.x) * (p2.x - p1.x), 1 / 2) / 3;
        }
    ...
    curve[1] = {
        ...
        y: interpolateValue(curve[0].y, interpolateValue(curve[3].y, curve[2].y, 3 / 2), 2 / 3)
    };
    var last = curve.length - 1;
    curve[last - 1] = {
        x: interpolateValue(curve[last].x, curve[last - 3].x, 1 / 3),
        y: interpolateValue(curve[last].y, interpolateValue(curve[last - 3].y, curve[last - 2].y, 3 / 2), 2 / 3)
    };
