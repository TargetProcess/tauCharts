...
export class BoxWhiskers extends Element {
    ...
    drawFrames(frames) {
        ...
        this.boxWhiskersDecorator(self.screenModel, fullData);
    }
    ...
    boxWhiskersDecorator(screenModel, data) {

        var model = screenModel.model;

        var cats = data.reduce((memo, row) => {
        ...
        var boxModel = this.buildRangeModel(screenModel, {
            barSize: 20,
            y0: 'Q25',
            y1: 'Q75',
            baseCssClass
        ...
        var boxes = this.config.options.container.selectAll('.box')
            .data(tuples);
        boxes.exit()
            .remove();
        boxes.call(createUpdateFunc(
        ...
        var whiskersModel = this.buildRangeModel(screenModel, {
            barSize: 2,
            y0: 'minimum',
            y1: 'maximum',
            baseCssClass
        ...
        var whiskers = this.config.options.container.selectAll('.whiskers')
            .data(tuples);
        whiskers.exit()
            .remove();
        whiskers.call(createUpdateFunc(
            this.config.guide.animationSpeed,
            null,
            whiskersModel
        ));
    ...
    buildRangeModel(screenModel, {barSize, y0, y1}) {
        var model;
        var xs = screenModel.model.scaleX;
        var ys = screenModel.model.scaleY;
        model = {
            x: ((d) => xs(d[xs.dim]) - barSize * 0.5),
            y: ((d) => ys(d[y1])),
            width: (() => barSize),
            height: ((d) => Math.abs(ys(d[y0]) - ys(d[y1])))
        };
