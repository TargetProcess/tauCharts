...
interface TimeDiffHighlightObject extends GrammarElement {
    _prepareData(data: any[], screenModel: ScreenModel): any[][];
    _createXIndex(data: any[], screenModel: ScreenModel): ({ind, val, pos})[];
}

const TimeDiffHighlight = <TimeDiffHighlightObject>{

    _prepareData(data, screenModel) {
        var groups = utils.groupBy(this.node().data(), screenModel.group);
        return Object
            .keys(groups)
            .sort((a, b) => {
    ...
    _createXIndex(data: any[], screenModel: ScreenModel) {
        return utils.unique(data.map((x) => {
            return x[screenModel.model.scaleX.dim] as number;
        }), String)
            .sort((x1, x2) => {
                return x1 - x2;
    ...
    draw() {
        ...
        const data = this._prepareData(node.data(), screenModel);
        const xIndex = this._createXIndex(data, screenModel);
    }
...
function TimeDiffTooltip(xSettings) {
    ...
    const tooltipExt: PluginObject = {
        ...
        onRender(chart) {
            ...
            units.forEach((u) => unitsGroupedData.set(u, getGroupedData(u.data(), u.screenModel)));
        }
